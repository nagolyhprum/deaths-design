...
### Phase 5 — Building variety + content *(complete as of 2026-04-22)*

> ⚠️ **MANUAL EDITOR STEP REQUIRED — tile set expansion and goal-room wiring:**
>
> 1. **⚠️ MANUAL EDITOR STEP REQUIRED — Tile set expansion per archetype**: Create distinct tile variants for each archetype in `building_tiles.tres` (e.g. wooden floor for HOUSE, stone floor for SHOP, carpet for APARTMENT). Update the WFC catalog in `wfc_room_generator.gd` with new source IDs for each variant and adjust socket values. The archetype is available on `BuildingGen.archetype` at generation time so future palette entries can filter by it.
>
> 2. **⚠️ MANUAL EDITOR STEP REQUIRED — Goal detection in the game scene**: Connect `WorldGen.goal_building_changed` to your game coordinator. Use it to display a destination marker or UI indicator. To detect when the player reaches the store, check when the player enters the goal building (the building with `is_goal=true`). See `WorldGen.goal_building` and `WorldGen.spawn_building` for the references.
>
> 3. **⚠️ MANUAL EDITOR STEP REQUIRED — Outdoor tile authoring**: The outdoor `TileMapLayer` uses source ID 0 with atlas coords (0,0) for path tiles and (1,0) for the goal-building entrance marker. Author these tiles in `outdoor_tiles.tres` (or configure `WorldGen.OUTDOOR_FLOOR_SOURCE_ID`). The goal entrance is marked with atlas (1,0) so it can be styled differently (e.g., a "STORE" sign overlay).

- [x] Multiple building archetypes (HOUSE, SHOP, APARTMENT, STORE) — `scripts/building_archetype.gd` with `room_weights_for()`, `min/max_floors()`, `basement_chance()`, `random_floor_count()`, `display_name()`; `archetype` and `is_goal` exports on `BuildingGen`
- [ ] ⚠️ MANUAL EDITOR STEP REQUIRED — Tile set expansion per archetype: add variant tiles to `building_tiles.tres` and new source IDs to the WFC catalog
- [x] Goal-structured buildings: spawn → store — `WorldGen` assigns STORE archetype + `is_goal=true` to the last building; `spawn_building` / `goal_building` references; `goal_building_changed` signal; `WorldGen._pick_archetype()` cycles HOUSE/SHOP/APARTMENT for intermediate buildings
- [x] Exterior / connector generation (streets, paths) — `WorldGen._generate_outdoor()` routes L-shaped axis-aligned tile paths between all consecutive building entrances; goal entrance marked with distinct atlas coord (1,0); `_route_path()` rewritten as L-path (horizontal first, then vertical)
- [x] Room type distribution reflects archetype — `RoomGraph.generate()` accepts `room_weights: Dictionary` from `BuildingArchetype.room_weights_for()`; `_shuffled_room_types()` upgraded to weighted pick; STORE archetype guarantees STORE rooms; HOUSE archetype excludes STORE rooms; zero-weight types never appear
...
- *2026-04-22* — Phase 5 implemented (code). `building_archetype.gd` added: `ArchetypeID` enum (HOUSE, SHOP, APARTMENT, STORE), `room_weights_for()` weight tables, `min/max_floors()`, `basement_chance()`, `random_floor_count()`, `display_name()`. `room_graph.gd` updated: `generate()` accepts `room_weights: Dictionary`; `_shuffled_room_types()` upgraded to archetype-weighted pick with fallback. `building_gen.gd` updated: `archetype` and `is_goal` exports; passes `BuildingArchetype.room_weights_for(archetype)` to both single-floor and multi-floor RoomGraph calls. `world_gen.gd` rewritten: assigns STORE+`is_goal=true` to last building, cycles HOUSE/SHOP/APARTMENT for others, `spawn_building`/`goal_building` references, `goal_building_changed` signal, improved L-path outdoor routing with goal entrance marker. Manual editor steps remain: tile set expansion per archetype, goal detection wiring, outdoor tile authoring.
