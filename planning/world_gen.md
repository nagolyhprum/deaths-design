...
- `TileMeta` autoload (`scripts/tile_meta.gd`) — shared enums for tile categories, sockets, and any other cross-scene tile metadata.
...
- **Camera framing:** single-room MVP keeps whole-room-in-frame. Camera-follow introduced when rooms exceed viewport.
...
### Phase 1 — MVP: seeded single room *(current)*

- [ ] Delete `ROOM_LAYOUT` constant and its iteration in `test_level.gd`
- [ ] Replace `test_level.gd` / `test_level.tscn` with `building_gen.gd` / `building_gen.tscn` (and `world_gen` as its parent)
- [ ] Add `TileMeta` autoload (`scripts/tile_meta.gd`) with `Category` and `Socket` enums
- [ ] Configure the interior TileSet: isometric mode, collision shapes per tile, custom data layers (`category`, `socket_n/e/s/w`, `weight`)
- [ ] Hand-author category + socket + weight values for every tile in the existing sheet
- [ ] `scripts/rng_streams.gd`: helper for splitting a seed into named sub-streams
- [ ] `scripts/wfc_room_generator.gd`: pure `generate(seed, tile_map_layer, size)` → populates the layer; bounded retry + fallback tile on contradiction
- [ ] `building_gen.gd`: `@tool`, exported `seed: int`, `@export_tool_button("Generate")`, `@export_tool_button("Randomize Seed")`; calls the generator against its active-floor `TileMapLayer`
- [ ] `world_gen.gd`: `@tool`, exported `world_seed: int`, generate/randomize buttons; for MVP just forwards a derived seed to its single child `building_gen`
- [ ] Add a second `TileMapLayer` for props on `building_gen` (above the floor/wall layer)
- [ ] Add prop-related custom data layers to the interior TileSet (`anchor`, `room_types`, `weight`, `clearance`) and author values for existing prop tiles (plant, small table, counter, etc.)
- [ ] `scripts/furniture_pass.gd`: pure function that reads WFC result + prop palette, places props on the props layer using a dedicated `furniture` RNG sub-stream, and runs flood-fill walkability validation with bounded retry
- [ ] Minimal Phase 1 prop palette: 1–2 props from the existing sheet (enough to prove the pipeline end-to-end)
- [ ] Runtime entry point calls the same `generate()` used by the tool button (WFC → furniture)
- [ ] Seed persistence across runtime launches (`user://` config)
- [ ] Verify: same seed reproduces same room *and* furniture in editor and runtime; different seeds produce different rooms

### Phase 2 — Multi-room layout (single floor)

- [ ] Building footprint + room graph (tier 1)
- [ ] Room type assignment (kitchen, hall, bedroom, …)
- [ ] Door placement between rooms, treated as WFC fixed constraints
- [ ] Per-room WFC stitched at shared walls
- [ ] Per-room-type prop palettes (kitchen → counter/stove/fridge, bedroom → bed/dresser, etc.)
- [ ] Furniture pass consumes room type to pick the right palette
- [ ] Connectivity validation via flood-fill from spawn (across rooms + through doors)
- [ ] `world_gen` places multiple buildings, routes outdoor tiles between entrances
- [ ] "Active building" ownership: `world_gen` toggles which building's floor is live
...
- WFC failure handling: reseed-per-room vs global restart — decide on first failure observed in Phase 1.
- Tile metadata storage: stay on plain-int custom data layers, or upgrade to a `TileMetadata` Resource for enum dropdowns in the TileSet editor — revisit if Phase 1 authoring gets painful.
