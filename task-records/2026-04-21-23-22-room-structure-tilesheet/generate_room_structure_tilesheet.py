...
def draw_wall(draw, ox, oy):
    ...
    draw.line(
        offset([(BASE_CENTER_X - HALF_W + 6, BASE_CENTER_Y - 6), (BASE_CENTER_X - 2, BASE_TOP_Y + 6)], ox, oy),
        fill=P["trim"],
        width=3,
    )


def draw_wall_insert(draw, ox, oy, variant):
    face = left_wall_face()
    draw_wall(draw, ox, oy)

    if variant == "window":
        frame = offset(quad_from_lerp(face, 0.26, 0.62, 0.58, 0.22), ox, oy)
        glass = offset(quad_from_lerp(face, 0.30, 0.58, 0.54, 0.26), ox, oy)
        draw_poly(draw, frame, P["trim"])
        draw_poly(draw, glass, P["window_glass"], P["trim_dark"])
        draw.line([glass[0], glass[2]], fill=P["window_hi"], width=2)
    elif variant == "door":
        jamb = offset(quad_from_lerp(face, 0.58, 0.94, 0.92, 0.56), ox, oy)
        door = offset(quad_from_lerp(face, 0.62, 0.90, 0.88, 0.60), ox, oy)
        panel = offset(quad_from_lerp(face, 0.68, 0.84, 0.82, 0.66), ox, oy)
        draw_poly(draw, jamb, P["trim"])
        draw_poly(draw, door, P["door"], P["trim_dark"])
        draw_poly(draw, panel, P["door_panel"], None)
        knob = (door[1][0] - 7, door[2][1] - 16)
        draw.ellipse((knob[0] - 2, knob[1] - 2, knob[0] + 2, knob[1] + 2), fill=P["frame"], outline=P["trim_dark"])
    elif variant == "picture":
        frame = offset(quad_from_lerp(face, 0.24, 0.54, 0.48, 0.28), ox, oy)
        art = offset(quad_from_lerp(face, 0.28, 0.50, 0.44, 0.32), ox, oy)
        draw_poly(draw, frame, P["frame"], P["trim"])
        draw_poly(draw, art, P["art"], None)
    elif variant == "double_window":
        left_frame = offset(quad_from_lerp(face, 0.18, 0.36, 0.32, 0.14), ox, oy)
        left_glass = offset(quad_from_lerp(face, 0.20, 0.34, 0.30, 0.16), ox, oy)
        right_frame = offset(quad_from_lerp(face, 0.42, 0.60, 0.56, 0.38), ox, oy)
        right_glass = offset(quad_from_lerp(face, 0.44, 0.58, 0.54, 0.40), ox, oy)
        draw_poly(draw, left_frame, P["trim"])
        draw_poly(draw, left_glass, P["window_glass"], P["trim_dark"])
        draw_poly(draw, right_frame, P["trim"])
        draw_poly(draw, right_glass, P["window_glass"], P["trim_dark"])
        draw.line([left_glass[0], left_glass[2]], fill=P["window_hi"], width=2)
        draw.line([right_glass[0], right_glass[2]], fill=P["window_hi"], width=2)
...
def draw_corner_wall(draw, ox, oy):
    ...
    draw_poly(draw, offset(cap, ox, oy), P["wall_top"])
    draw.line(
        offset([(BASE_CENTER_X + 2, BASE_TOP_Y + 6), (BASE_CENTER_X + HALF_W - 6, BASE_CENTER_Y - 6)], ox, oy),
        fill=P["trim"],
        width=3,
    )
    pillar = offset(
...
def draw_rug(draw, ox, oy):
    outer = offset(
        [
            (BASE_CENTER_X, BASE_TOP_Y + 10),
            (BASE_CENTER_X + 48, BASE_CENTER_Y),
    ...
    inner = offset(
        [
            ...
            (BASE_CENTER_X + 40, BASE_CENTER_Y),
    ...
    draw_poly(draw, inner, P["rug_main"], P["rug_trim"])
...
def draw_table(draw, ox, oy, small=False):
    ...
    top = offset(
        [(BASE_CENTER_X, top_y - top_h), (BASE_CENTER_X + top_w, top_y), (BASE_CENTER_X, top_y + top_h), (BASE_CENTER_X - top_w, top_y)],
        ox,
        oy,
    )
    left = offset(
        [(BASE_CENTER_X - top_w, top_y), (BASE_CENTER_X, top_y + top_h), (BASE_CENTER_X, top_y + top_h + 12), (BASE_CENTER_X - top_w, top_y + 12)],
        ox,
        oy,
    )
    right = offset(
        [(BASE_CENTER_X, top_y + top_h), (BASE_CENTER_X + top_w, top_y), (BASE_CENTER_X + top_w, top_y + 12), (BASE_CENTER_X, top_y + top_h + 12)],
        ox,
        oy,
    )
...
render_tile(0, lambda d, ox, oy: draw_wall(d, ox, oy))
render_tile(1, lambda d, ox, oy: draw_wall_insert(d, ox, oy, "window"), alt=True)
render_tile(2, lambda d, ox, oy: draw_wall_insert(d, ox, oy, "door"))
render_tile(3, lambda d, ox, oy: draw_counter(d, ox, oy), alt=True)
render_tile(4, lambda d, ox, oy: draw_rug(d, ox, oy))
render_tile(5, lambda d, ox, oy: draw_table(d, ox, oy), alt=True)
render_tile(6, lambda d, ox, oy: draw_plant(d, ox, oy))
render_tile(7, lambda d, ox, oy: draw_wall_insert(d, ox, oy, "picture"), alt=True)
render_tile(8, lambda d, ox, oy: draw_corner_wall(d, ox, oy))
render_tile(9, lambda d, ox, oy: draw_wall_insert(d, ox, oy, "double_window"), alt=True)
render_tile(10, lambda d, ox, oy: draw_counter(d, ox, oy, corner=True))
render_tile(11, lambda d, ox, oy: draw_table(d, ox, oy, small=True), alt=True)
