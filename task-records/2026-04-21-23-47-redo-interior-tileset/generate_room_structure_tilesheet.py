...
def draw_upper_left_wall(draw, ox, oy, variant="plain"):
    ...
    if variant == "window":
        window = offset(
            [
                (BASE_CENTER_X - 38, BASE_CENTER_Y - WALL_H + 18),
                (BASE_CENTER_X - 4, BASE_TOP_Y - WALL_H + 24),
                (BASE_CENTER_X - 4, BASE_TOP_Y - 6),
                (BASE_CENTER_X - 38, BASE_CENTER_Y - 2),
            ],
            ox,
            oy,
        )
        draw_poly(draw, window, P["window_glass"], P["trim"])
        draw.line([window[0], window[2]], fill=P["trim"], width=2)
    elif variant == "door":
        door = offset(
            [
                (BASE_CENTER_X - 26, BASE_CENTER_Y - WALL_H + 18),
                (BASE_CENTER_X - 2, BASE_TOP_Y - WALL_H + 22),
                (BASE_CENTER_X - 2, BASE_TOP_Y + 8),
                (BASE_CENTER_X - 26, BASE_CENTER_Y + 8),
            ],
            ox,
            oy,
        )
        draw_poly(draw, door, P["door"], P["trim"])
        knob = (door[1][0] - 8, door[2][1] - 18)
        draw.ellipse([knob[0] - 2, knob[1] - 2, knob[0] + 2, knob[1] + 2], fill=P["frame"], outline=P["trim"])
    elif variant == "picture":
        frame = offset(
            [
                (BASE_CENTER_X - 34, BASE_CENTER_Y - WALL_H + 18),
                (BASE_CENTER_X - 6, BASE_TOP_Y - WALL_H + 24),
                (BASE_CENTER_X - 6, BASE_TOP_Y - WALL_H + 48),
                (BASE_CENTER_X - 34, BASE_CENTER_Y - WALL_H + 42),
            ],
            ox,
            oy,
        )
        draw_poly(draw, frame, P["frame"])
        inner = [(x + 2, y + 2) for x, y in frame]
        draw_poly(draw, inner, P["art"], None)
...
def draw_upper_right_wall(draw, ox, oy, variant="plain"):
    ...
    if variant == "window":
        ...
        draw_poly(draw, window, P["window_glass"], P["trim"])
        draw.line([window[1], window[3]], fill=P["trim"], width=2)
    elif variant == "door":
        door = offset(
            [
                (BASE_CENTER_X + 2, BASE_TOP_Y - WALL_H + 22),
        ...
        draw_poly(draw, door, P["door_panel"], P["trim"])
        knob = (door[0][0] + 8, door[3][1] - 18)
        draw.ellipse([knob[0] - 2, knob[1] - 2, knob[0] + 2, knob[1] + 2], fill=P["frame"], outline=P["trim"])
    elif variant == "picture":
        frame = offset(
            [
        ...
        draw_poly(draw, frame, P["frame"])
        inner = [(x - 2 if i in (1, 2) else x + 2, y + 2) for i, (x, y) in enumerate(frame)]
        draw_poly(draw, inner, P["art"], None)
